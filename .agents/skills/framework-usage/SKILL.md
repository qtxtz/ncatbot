...
| 分层命令结构（子命令/命令组） | CommandGroup + CommandGroupHook | [hooks.md](./references/hooks.md) / `examples/common/08_command_group/` |
...
| 平台登录/配置/认证 | 各适配器登录流程 | `docs/guide/adapter/` |
...
| 问题 | 首先检查 |
|------|---------|
| Bot 启动失败 | `ncatbot config check`，NapCat 是否运行，ws_uri 格式。各平台详见 `docs/guide/adapter/` |
