...
各适配器通过 `cli_configure()` 类方法为 `ncatbot init` / `adapter enable` 提供交互式配置。含智能跳过逻辑（如 NapCat 自动安装跳过 WS/WebUI 输入、Bilibili 扫码跳过凭据输入）。

→ 详见 [references/adapter-cli-hooks.md](./references/adapter-cli-hooks.md)（实现位置、跳过设计、新适配器实现规则）
