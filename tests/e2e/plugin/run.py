...
PLUGIN_TEST_INFO: dict[str, dict] = {
    "hello_world": {
        "description": "最小可运行插件 — 回复 hello / hi",
        "test_instructions": [
            "群里发送「hello」→ 应收到回复",
            "群里发送「hi」→ 应收到富文本快速回复",
            "私聊发送「hello」→ 应收到私聊回复",
    ...
    "message_types": {
        ...
        "test_instructions": [
            "群里发送「图文」→ 应收到图文混排消息",
            "群里发送「图片」→ 应收到 URL 格式图片",
            "群里发送「视频」→ 应收到视频消息",
            "群里发送「文件视频」→ 应收到文件视频",
            "群里发送「表情」→ 应收到动画表情",
            "群里发送「at我」→ 应收到 @你 的消息",
            "群里发送「转发」→ 应收到合并转发消息",
            "群里发送「嵌套转发」→ 应收到嵌套合并转发消息",
    ...
    "bot_api": {
        ...
        "test_instructions": [
            ...
            "群里发送「解禁 @某人」→ 应解除禁言",
            "回复一条消息并发送「转发」→ 应输出被回复消息的 raw_message",
        ],
    },
    "hook_and_filter": {
    ...
    "group_manager": {
        ...
        "test_instructions": [
            ...
            "群里发送「全体禁言」→ 应开启全体禁言",
            "群里发送「解除全体禁言」→ 应关闭全体禁言",
            "群里发送「改名片 @某人 新名片」→ 应修改群名片",
            "群里发送「设置欢迎语 欢迎新人!」→ 应修改欢迎消息",
            "群里发送「踢 @某人」→ 应踢出用户 (⚠ 谨慎测试)",
        ],
    },
    "qa_bot": {
    ...
    "full_featured_bot": {
        ...
        "test_instructions": [
            ...
            "群里发送「查看配置」→ 应显示当前配置",
            "群里发送「授权 @自己」→ 应授予管理权限",
            "群里发送「管理踢 @某人」→ 应踢出用户 (⚠ 谨慎测试)",
            "群里发送「管理禁言 @某人 10」→ 应禁言 10 秒",
            "群里发送「开启早安」→ 应启动定时早安任务 (07:30)",
            "群里发送「关闭早安」→ 应停止定时早安任务",
        ],
    },
}
