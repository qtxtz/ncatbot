"""
qq/04_bot_api — QQ Bot API 使用大全

演示功能:
  - self.api.qq 消息发送 (post_group_msg / send_group_image)
  - self.api.qq.manage 群管理 (禁言 / 踢人)
  - self.api.qq.query 信息查询 (群列表 / 成员信息 / 登录信息)
  - 消息语法糖 (text / image / at / reply 关键字参数)
  - @registrar.qq.on_group_command() 命令匹配 + At/int 参数绑定
...
class BotAPIPlugin(NcatBotPlugin):
    ...
    @registrar.qq.on_group_command("查登录信息")
    ...
    @registrar.qq.on_group_command("发图片")
    ...
    async def on_send_file(self, event: GroupMessageEvent):
        ...
        await self.api.qq.send_group_file(
            event.group_id, str(EXAMPLE_FILE), name="示例文件.pdf"
        )
