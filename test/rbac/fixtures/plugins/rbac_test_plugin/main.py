...
class RBACTestPlugin(NcatBotPlugin):
    ...
    @group_filter
    @command_registry.command(
        name="group_admin_cmd",
        description="群聊管理员命令"
    )
    async def group_admin_cmd(self, event: MessageEvent):
        """群聊管理员命令"""
        self.command_executions.append(("group_admin_cmd", event.user_id, event.group_id))
        await event.reply("group_admin_cmd executed")
    
    @root_filter
    @private_filter
    @command_registry.command(
        name="private_root_cmd",
        description="私聊 Root 命令"
    )
