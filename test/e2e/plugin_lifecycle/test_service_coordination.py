...
class TestRegisterHandlerCoordination:
    ...
    def test_handler_priority_respected(self, test_suite, handler_plugin_name):
        """测试事件处理器优先级被尊重"""
    ...
    def test_handler_unsubscribed_on_plugin_unload(
    ...
    def test_custom_events_received(self, test_suite, handler_plugin_name):
        """测试自定义事件被接收"""
        ...
        run_coroutine(publish_custom)
        time.sleep(0.02)

        # 验证收到自定义事件
        assert len(HandlerTestPlugin.custom_events) >= 1

    def test_handler_not_called_after_unregister(
        self, test_suite, handler_plugin_name
    ):
        """测试取消注册后处理器不被调用"""
        plugin = test_suite.register_plugin_sync(handler_plugin_name)
        ...
        # 发送消息
        test_suite.inject_group_message_sync("Before unregister")
        time.sleep(0.02)
        ...
        # 发送消息
        test_suite.inject_group_message_sync("After unregister")
        time.sleep(0.02)
...
class TestUnifiedRegistryCoordination:
    ...
    def test_command_dispatch_through_unified_registry(
    ...
    def test_command_with_arguments(self, test_suite, command_plugin_name):
        """测试带参数的命令"""
        test_suite.register_plugin_sync(command_plugin_name)
        CommandTestPlugin = get_plugin_class(command_plugin_name)

        # 发送带参数的命令
        test_suite.inject_group_message_sync("cmd_echo hello_world")
        time.sleep(0.02)
    ...
    def test_command_prefix_handling(self, test_suite, command_plugin_name):
        """测试命令前缀处理"""
        test_suite.register_plugin_sync(command_plugin_name)
        CommandTestPlugin = get_plugin_class(command_plugin_name)
    ...
    def test_registry_reinitialized_on_plugin_change(
        self, test_suite, command_plugin_name
    ):
...
class TestPluginConfigServiceCoordination:
    ...
    def test_config_isolation_between_plugins(
        self, test_suite, config_plugin_name, handler_plugin_name
    ):
        """测试插件间配置隔离"""
    ...
    def test_config_persistence_across_operations(self, test_suite, config_plugin_name):
        """测试配置跨操作持久化"""
        test_suite.register_plugin_sync(config_plugin_name)
    ...
    def test_config_defaults_on_first_load(self, test_suite, config_plugin_name):
        """测试首次加载时使用默认值"""
        test_suite.register_plugin_sync(config_plugin_name)
    ...
    def test_config_modification_via_plugin(self, test_suite, config_plugin_name):
        """测试通过插件修改配置"""
        test_suite.register_plugin_sync(config_plugin_name)
        ConfigTestPlugin = get_plugin_class(config_plugin_name)

        # 通过命令修改配置
        test_suite.inject_group_message_sync("cfg_set api_key new_key")
        time.sleep(0.02)
...
class TestMultiServiceCoordination:
    ...
    def test_all_services_work_together(self, test_suite, full_feature_plugin_name):
        """测试所有服务协同工作"""
    ...
    def test_services_cleanup_on_unload(self, test_suite, full_feature_plugin_name):
        """测试卸载时所有服务清理 - 端到端验证"""
    ...
    def test_config_command_interaction(self, test_suite, full_feature_plugin_name):
        """测试配置与命令交互"""
        test_suite.register_plugin_sync(full_feature_plugin_name)
        FullFeaturePlugin = get_plugin_class(full_feature_plugin_name)

        # 通过命令设置配置
        test_suite.inject_group_message_sync("ff_config set feature_value new_value")
        time.sleep(0.02)
        test_suite.assert_reply_sent()
        ...
        # 通过命令获取配置
        test_suite.clear_call_history()
        test_suite.inject_group_message_sync("ff_config get feature_value")
        time.sleep(0.02)
        test_suite.assert_reply_sent()
...
class TestEdgeCases:
    ...
    def test_load_plugin_twice(self, test_suite, command_plugin_name):
        """测试重复加载同一插件"""
        test_suite.register_plugin_sync(command_plugin_name)
    ...
    def test_unload_nonexistent_plugin(self, test_suite):
        """测试卸载不存在的插件"""
        # 应该不会报错
        test_suite.unregister_plugin_sync("nonexistent_plugin")

    def test_rapid_load_unload_cycles(
        self, test_suite, command_plugin_name, handler_plugin_name
    ):
        """测试快速加载卸载循环"""
        for _ in range(3):
            test_suite.register_plugin_sync(command_plugin_name)
    ...
    def test_event_during_plugin_operations(
        self, test_suite, handler_plugin_name
    ):
        """测试插件操作期间的事件处理"""
        test_suite.register_plugin_sync(handler_plugin_name)
        ...
        # 发送事件
        test_suite.inject_group_message_sync("Test message 1")
