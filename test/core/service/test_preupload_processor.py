...
from ncatbot.core.service.builtin.preupload.processor import (
    MessagePreUploadProcessor,
    ProcessResult,
)
from ncatbot.core.service.builtin.preupload.client import UploadResult
...
def mock_upload_service():
    """创建模拟的上传服务"""
    service = MagicMock()
    service.upload_file = AsyncMock(return_value=UploadResult(
        success=True,
        file_path="/uploaded/path/file.jpg",
        file_size=1024,
        sha256="abc123"
    ))
    service.upload_bytes = AsyncMock(return_value=UploadResult(
        success=True,
...
class TestForwardMessageProcessing:
    ...
    @pytest.mark.asyncio
    async def test_process_deeply_nested_forward(
        self, processor, mock_upload_service
    ):
        """测试处理深度嵌套的转发消息"""
        data = {
