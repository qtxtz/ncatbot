...
from ncatbot.core.service.builtin.plugin_config import (
    PluginConfigService,
    PluginConfig,
    ConfigPersistence,
)
...
def config_service(temp_config_file):
    """创建配置服务实例"""
    with patch("ncatbot.core.service.builtin.plugin_config.service.CONFIG_PATH", temp_config_file):
...
class TestPluginConfigService:
    ...
    async def test_set_with_on_change_callback(self, config_service):
        ...
        config_service.register_config(
            "plugin", "key", "default", on_change=on_change
        )
