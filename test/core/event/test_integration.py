...
class TestCompleteMessageParsing:
    ...
    def test_parse_forward_message(self):
        """测试解析转发消息"""
        message_data = [
            {"type": "forward", "data": {"id": "7589518469182843880"}}
        ]
...
class TestTypeMapCompleteness:
    ...
    def test_all_types_registered(self):
        """测试所有类型都已注册"""
        expected_types = [
            "text", "face", "at", "reply",  # primitives
            "image", "record", "video", "file",  # media
            "share", "location", "music", "json", "markdown",  # misc
            "forward",  # forward
        ]
