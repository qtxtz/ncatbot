...
class TestEventDispatcherDispatch:
    ...
    @pytest.mark.asyncio
    async def test_dispatch_valid_message_event(self, event_dispatcher, sample_message_event_data):
        """分发有效消息事件"""
        published_events = []
    ...
    @pytest.mark.asyncio
    async def test_dispatch_valid_notice_event(self, event_dispatcher, sample_notice_event_data):
        """分发有效通知事件"""
        published_events = []
    ...
    @pytest.mark.asyncio
    async def test_dispatch_valid_request_event(self, event_dispatcher, sample_request_event_data):
        """分发有效请求事件"""
        published_events = []
    ...
    @pytest.mark.asyncio
    async def test_dispatch_valid_meta_event(self, event_dispatcher, sample_meta_event_data):
        """分发有效元事件"""
        published_events = []
    ...
    async def test_dispatch_unknown_event(self, event_dispatcher):
        ...
        # 订阅所有可能的事件类型
        for event_type in ["ncatbot.message_event", "ncatbot.notice_event", "ncatbot.request_event", "ncatbot.meta_event"]:
    ...
    @pytest.mark.asyncio
    async def test_dispatch_parse_error(self, event_dispatcher, sample_message_event_data):
        """解析失败时记录警告"""
        with patch("ncatbot.core.client.dispatcher.EventParser") as mock_parser:
    ...
    @pytest.mark.asyncio
    async def test_dispatch_general_exception(self, event_dispatcher, sample_message_event_data):
        """一般异常处理"""
        with patch("ncatbot.core.client.dispatcher.EventParser") as mock_parser:
...
class TestEventDispatcherCallable:
    ...
    @pytest.mark.asyncio
    async def test_callable_interface(self, event_dispatcher, sample_message_event_data):
        """__call__ 方法可作为回调"""
        published_events = []
    ...
    @pytest.mark.asyncio
    async def test_callable_equals_dispatch(self, event_dispatcher, sample_message_event_data):
        ...
        with patch("ncatbot.core.client.dispatcher.EventParser") as mock_parser:
            ...
            # 使用 dispatch
            event_dispatcher.event_bus.subscribe("ncatbot.message_event", capture_dispatch)
            await event_dispatcher.dispatch(sample_message_event_data)
...
class TestEventDispatcherIntegration:
    ...
    @pytest.mark.asyncio
    async def test_dispatch_creates_ncatbot_event(self, event_dispatcher, sample_message_event_data):
    ...
    @pytest.mark.asyncio
    async def test_dispatch_passes_api_to_parser(self, event_dispatcher, sample_message_event_data, mock_api):
        """分发时将 API 传递给解析器"""
        with patch("ncatbot.core.client.dispatcher.EventParser") as mock_parser:
            ...
            mock_parser.parse.assert_called_once_with(
                sample_message_event_data, 
                event_dispatcher.api
            )
