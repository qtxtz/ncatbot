...
class MessageActionMixin(ContextMixin):
    ...
    async def reply(self, text: str, **kwargs):
        if getattr(self, "message_type", "") == "group" and hasattr(self, "group_id"):
            return await self.api.post_group_msg(group_id=self.group_id, text=text, **kwargs)
        elif getattr(self, "message_type", "") == "private" and hasattr(self, "user_id"):
            return await self.api.post_private_msg(user_id=self.user_id, text=text, **kwargs)
...
class GroupAdminMixin(ContextMixin):
    ...
    async def kick(self, reject_add_request: bool = False):
        if hasattr(self, "group_id") and hasattr(self, "user_id"):
            return await self.api.set_group_kick(self.group_id, self.user_id, reject_add_request)
...
class RequestActionMixin(ContextMixin):
    ...
    async def approve(self, remark: str = "", reason: str = ""):
        ...
        if request_type == "friend":
            return await self.api.set_friend_add_request(flag=flag, approve=True, remark=remark)
        elif request_type == "group":
            sub_type = getattr(self, "sub_type", "")
            return await self.api.set_group_add_request(flag=flag, sub_type=sub_type, approve=True, reason="")
    ...
    async def reject(self, reason: str = ""):
        ...
        if request_type == "friend":
            ...
            sub_type = getattr(self, "sub_type", "")
            return await self.api.set_group_add_request(flag=flag, sub_type=sub_type, approve=False, reason=reason)