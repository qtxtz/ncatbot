...
class EventRegistry:
    ...
    def add_group_message_handler(self, handler: Callable, filter: Optional[Type[MessageSegment]] = None):
        """添加群消息处理器，自动过滤群消息事件"""
    ...
    def add_private_message_handler(self, handler: Callable, filter: Optional[Type[MessageSegment]] = None):
    ...
    def add_request_handler(self, handler: Callable, filter: Optional[Literal["group", "friend"]] = None):
        """[弃用] 添加请求事件处理器
    ...
    def add_startup_handler(self, handler: Callable):
        ...
        def filter_func(event):
            return (event.meta_event_type == 'lifecycle' and hasattr(event, 'sub_type') and event.sub_type == 'connect')
    ...
    def add_heartbeat_handler(self, handler: Callable):
        """添加心跳事件处理器，自动过滤心跳事件"""
        def filter_func(event):
            return (hasattr(event, 'meta_event_type') and event.meta_event_type == 'heartbeat')
    ...
    def on_group_request(self, filter: Optional[Callable[[GroupRequestEvent], bool]] = None):
        def decorator(f):
