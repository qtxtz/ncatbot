...
from .utils import (
    is_local_file, is_base64_data, is_remote_url,
    get_local_path, extract_base64_data, generate_filename_from_type,
)
...
class PreUploadService(BaseService):
    ...
    async def process_message_array(self, messages: List[Dict[str, Any]]) -> ProcessResult:
        """处理消息数组"""
        if not self._message_processor:
            return ProcessResult(success=False, errors=["预上传服务未初始化"])
    ...
    async def preupload_file(self, file_value: str, file_type: str = "file") -> PreUploadResult:
    ...
    async def preupload_file_if_needed(self, file_value: str, file_type: str = "file") -> str:
