    ...
    def validate_function_decorators(func: Callable):
        ...
        for i, option in enumerate(options):
            ...
            if option.get('short_name'):
                ...
                if not short_name.isalnum() or len(short_name) != 1:
                    errors.append(f"{option_info}: 短选项名 '{short_name}' 必须是单个字母或数字")
                
                # 冲突检查
                if short_name in short_options:
                    errors.append(f"{option_info}: 短选项名 '{short_name}' 与选项 #{short_options[short_name]['index']+1} 冲突")
                else:
            ...
            if option.get('long_name'):
                ...
                if not long_name.replace('-', '').replace('_', '').isalnum():
                    errors.append(f"{option_info}: 长选项名 '{long_name}' 只能包含字母、数字、连字符和下划线")
                
                if long_name.startswith('--'):
                    errors.append(f"{option_info}: 长选项名 '{long_name}' 不应包含 '--' 前缀")
                    warnings.append(f"{option_info}: 建议将 '{long_name}' 改为 '{long_name.lstrip('-')}'")
                
                if len(long_name) < 2:
                    errors.append(f"{option_info}: 长选项名 '{long_name}' 至少需要两个字符")
                
                # 冲突检查
                if long_name in long_options:
                    errors.append(f"{option_info}: 长选项名 '{long_name}' 与选项 #{long_options[long_name]['index']+1} 冲突")
                else:
                    long_options[long_name] = {'index': i, 'option': option}
                    all_option_names.add(long_name)
            
            # 检查选项是否为空
            if not option.get('short_name') and not option.get('long_name'):
        ...
        for i, param in enumerate(params):
            ...
            if not name.replace('_', '').replace('-', '').isalnum():
                errors.append(f"{param_info}: 参数名 '{name}' 只能包含字母、数字、连字符和下划线")
            
            # 冲突检查
            if name in param_names:
                errors.append(f"{param_info}: 参数名 '{name}' 与参数 #{param_names[name]['index']+1} 冲突")
            else:
        ...
        for i, group in enumerate(groups):
            ...
            if name in group_names:
                errors.append(f"{group_info}: 选项组名 '{name}' 与选项组 #{group_names[name]['index']+1} 冲突")
            else:
            ...
            if not choices:
                ...
                if default and default not in choices:
                    errors.append(f"{group_info}: 选项组 '{name}' 的默认值 '{default}' 不在选项列表中")
                ...
                for choice in choices:
                    if choice in all_group_choices:
                        ...
                        for other_group_name, other_group_info in group_names.items():
                            if choice in other_group_info['group'].get('choices', []):
                                conflicting_group = f"选项组 '{other_group_name}'"
                                break
                        if conflicting_group:
                            errors.append(f"{group_info}: 选项组 '{name}' 的值 '{choice}' 与 {conflicting_group} 的值冲突")
                        else:
                            errors.append(f"{group_info}: 选项组 '{name}' 的值 '{choice}' 与其他选项组的值冲突")
                ...
                for choice in choices:
                    if choice in short_options:
                        ...
                        errors.append(f"{group_info}: 选项组 '{name}' 的值 '{choice}' 与短选项 '{choice}' (选项 #{conflicting_option['index']+1}) 冲突")
                
                # 检查选项组的值与长选项名冲突
                for choice in choices:
                    if choice in long_options:
                        conflicting_option = long_options[choice]
                        errors.append(f"{group_info}: 选项组 '{name}' 的值 '{choice}' 与长选项 '{choice}' (选项 #{conflicting_option['index']+1}) 冲突")
                
                # 检查选项组的值与参数名冲突
                for choice in choices:
                    if choice in param_names:
                        conflicting_param = param_names[choice]
                        errors.append(f"{group_info}: 选项组 '{name}' 的值 '{choice}' 与参数 '{choice}' (参数 #{conflicting_param['index']+1}) 冲突")
        ...
        if errors:
            ...
            if "选项组值" in '; '.join(errors) and "冲突" in '; '.join(errors):
                suggestions.append("检查选项组的值是否与其他选项组的值、选项名或参数名冲突，确保所有名称都是唯一的")
