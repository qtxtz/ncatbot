...
class ParamsValidator:
    ...
    def analyze_params(self):
        ...
        earliest = min(decorated_param_indexes) if decorated_param_indexes else len(actual_params)
        trailing_range = set(range(earliest, len(actual_params)))
        if decorated_param_indexes and not decorated_param_indexes.issuperset(trailing_range):
            # 找出位于尾部却未被装饰的参数
            offending = [actual_params[i].name for i in sorted(trailing_range - decorated_param_indexes)]
            raise CommandRegistrationError(
                self.func.__name__,
                "装饰器参数必须位于参数列表尾部且连续",
    ...
    def detect_args_type(self) -> Tuple[List[type], List[bool]]:
        ...
        for param in param_list:
            ...
            if annotation == inspect.Parameter.empty:
                LOG.error(f"函数参数缺少类型注解: {self.descriptor.func_qualname} 的参数 '{param.name}' 缺少类型注解")
                LOG.info(f"函数来自 {self.descriptor.func_module}.{self.descriptor.func_qualname}")
                raise ValueError(f"函数参数缺少类型注解: {self.descriptor.func_qualname} 的参数 '{param.name}' 缺少类型注解")
            ...
            if annotation in (str, int, float, bool):
                ...
                LOG.error(f"函数参数类型不支持: {self.descriptor.func_qualname} 的参数 '{param.name}' 的类型注解 {annotation} 不支持")
                LOG.info(f"函数来自 {self.descriptor.func_module}.{self.descriptor.func_qualname}")
                LOG.info(f"支持的类型: str, int, float, bool, Sentence 或 MessageSegment 的子类")
                raise ValueError(f"函数参数类型不支持: {self.descriptor.func_qualname} 的参数 '{param.name}' 的类型注解 {annotation} 不支持，"
