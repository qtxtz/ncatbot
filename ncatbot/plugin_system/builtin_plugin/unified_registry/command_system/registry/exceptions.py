...
class TypeConversionError(ValidationError):
    ...
    def __init__(self, param_name: str, value: Any, target_type: str, conversion_errors: List[str], **kwargs):
        self.conversion_errors = conversion_errors
        
        details = f"值 '{value}' (类型: {type(value).__name__}) 无法转换为 {target_type}"
...
class MultiTypeConversionError(TypeConversionError):
    ...
    def __init__(self, param_name: str, value: Any, supported_types: List[str], 
                 type_errors: Dict[str, str], type_hints: Dict[str, str] = None, **kwargs):
        ...
        # 先调用父类构造函数
        super().__init__(
            param_name, value, f"({' 或 '.join(supported_types)})", 
            list(type_errors.values()), **kwargs
        )
...
class TooManyArgumentsError(ArgumentError):
    ...
    def __init__(self, command_name: str, expected_count: int, actual_count: int, **kwargs):
...
class ErrorHandler:
    ...
    def format_error(self, error: CommandSystemError, context: Optional[ErrorContext] = None) -> str:
        """格式化错误信息为用户友好的格式"""
        formatter = self.error_formatters.get(type(error), self._format_default)
        return formatter(error, context)
    
    def _format_command_not_found(self, error: CommandNotFoundError, context: Optional[ErrorContext]) -> str:
        """格式化命令不存在错误"""
        msg = f"❌ {error.message}\n"
        
        if error.available_commands:
            # 显示相似命令
            similar = [cmd for cmd in error.available_commands 
                      if self._calculate_similarity(error.command_name, cmd) > 0.5][:3]
            if similar:
    ...
    def _format_argument_error(self, error: ArgumentError, context: Optional[ErrorContext]) -> str:
        """格式化参数错误"""
        msg = f"❌ {error.message}\n"
        if error.details:
            msg += f"📝 {error.details}\n"
    ...
    def _format_validation_error(self, error: ValidationError, context: Optional[ErrorContext]) -> str:
        """格式化验证错误"""
        msg = f"❌ 参数验证失败\n"
    ...
    def _format_multi_type_error(self, error: MultiTypeConversionError, context: Optional[ErrorContext]) -> str:
        """格式化多类型转换错误"""
    ...
    def _format_default(self, error: CommandSystemError, context: Optional[ErrorContext]) -> str:
        """默认错误格式化"""
