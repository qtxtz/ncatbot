...
class NcatBotPlugin(BasePlugin, TimeTaskMixin, ConfigMixin):
    ...
    async def __onload__(self) -> None:
        ...
        self.services.bot_client.registry_engine.handle_plugin_load()
    ...
    async def __unload__(self, *a: Any, **kw: Any) -> None:
        ...
        try:
            ...
            self.services.bot_client.registry_engine.handle_plugin_unload(self.name)

            # 清理新 Dispatcher 和 Session
            self.services.bot_client.handler_dispatcher.revoke_plugin(self.name)
            if self.services.session_manager:
                self.services.session_manager.revoke_plugin(self.name)
