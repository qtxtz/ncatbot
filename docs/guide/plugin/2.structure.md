...
> 示例参考：[examples/05_bot_api/](../../../examples/05_bot_api/) 包含 `resources/` 目录。
...
> 取自 [examples/14_external_api/manifest.toml](../../../examples/14_external_api/manifest.toml)。
...
> 依赖解析的详细机制参见 [高级主题 — 插件依赖管理](7.advanced.md#插件依赖管理)。
...
> 各 Mixin 的详细 API 参见 [Mixin 能力体系](mixins.md)。
...
> 框架会将插件根目录（`plugins/`）添加到 `sys.path`，因此每个插件文件夹相当于一个 Python 包。详细的跨插件导入机制参见 [高级主题 — 跨插件交互](advanced.md#跨插件交互)。

---

## 下一步

- [生命周期](lifecycle.md) — 了解 `on_load()` / `on_close()` 在整个加载流程中的位置
- [事件处理](event-handling.md) — 学习用装饰器注册事件处理器
- [Mixin 能力体系](mixins.md) — 使用配置、数据、权限、定时任务等 Mixin
