...
> **提示**：如果你还没有 uv，也可以直接用 pip。NapCat 在首次运行时会自动下载配置。
...
> 将 `bot_uin` 替换为你的实际 QQ 号。其他配置项保持默认即可，完整配置说明参见 [配置管理](../plugin/mixins.md#configmixin--配置持久化)。
...
> 完整字段说明参见 [插件结构 — manifest.toml 详解](structure.md#manifesttoml-详解)。
...
> 完整源码：[examples/01_hello_world/](../../../examples/01_hello_world/)
...
| `self.api.post_group_msg(group_id, text="...")` | 直接调用 API 发送群消息 |
| `get_log("name")` | 获取日志实例 |
...
- [插件结构](structure.md) — 深入了解 manifest.toml 字段、基类选择、目录布局
- [生命周期](lifecycle.md) — 理解插件的加载/卸载流程
- [事件处理](event-handling.md) — 掌握三种事件消费模式
